...
function loadTemplate(name) {
  ...
  try {
    // Git for Windows may check templates out with CRLF; normalize before parsing
    // so the marker format behaves identically on every supported platform.
    const raw = fs.readFileSync(path.join(PROMPTS_DIR, name), 'utf8').replace(/\r\n/g, '\n');
