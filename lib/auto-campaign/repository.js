...
function markDispatchedIfAllowed(id, runId, leaseMs = 120000) {
  const db = getDb();
  return db.transaction(() => {
    ...
    if (result.changes === 1) {
      ...
      `).run(now + leaseMs, now, run.account_uid, runId);
    }
    return {
