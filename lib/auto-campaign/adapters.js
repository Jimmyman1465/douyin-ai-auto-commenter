...
function createDouyinAdapter(ctx) {
  return {
    ...
    async supportsBatch() {
      return Boolean(await ctx.loggedCall(
        'quick_comment_batch_capability',
        {},
        'typeof window.__bridge.publishBatch === "function"',
      ));
    },

    async publishBatch(items, options = {}) {
      const payload = items.map(item => ({
        awemeId: String(item.awemeId),
        text: String(item.text),
      }));
      const intervalMinMs = Math.max(1000, Number(options.intervalMinMs) || 1000);
      const intervalMaxMs = Math.max(intervalMinMs, Number(options.intervalMaxMs) || intervalMinMs);
      const timeout = Math.max(35000, Number(options.timeout) || 35000);
      const expression = `window.__bridge.publishBatch(${JSON.stringify(payload)}, ${JSON.stringify({
        intervalMinMs,
        intervalMaxMs,
        stopOnError: true,
      })})`;
      return ctx.loggedCall('quick_comment_batch', {
        target_count: payload.length,
